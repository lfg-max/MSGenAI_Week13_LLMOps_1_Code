...
def get_judge_llm(temperature=1.0, overrides: Dict[str, Any] = None):
    """
    Initialize and return the LLM Judge instance.

    Creates a dedicated Azure OpenAI model (typically GPT-5-mini) for evaluating
    generated code. Uses temperature=1.0 for gpt-5-mini compatibility.

    Args:
        temperature (float): Sampling temperature. Default 1.0 for gpt-5-mini compatibility.
        overrides (Dict[str, Any], optional): Override config values from UI.
    ...
    def execute_query(self, user_query: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.error(f"Gen/Eval Error: {e}", exc_info=True)
                return {"response": f"Error generating or evaluating code: {str(e)}"}
